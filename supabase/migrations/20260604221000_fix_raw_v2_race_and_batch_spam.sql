-- ============================================================
-- MIGRAÇÃO: 20260604221000_fix_raw_v2_race_and_batch_spam.sql
-- DATA:     2026-06-04
-- MOTIVO:   Auditoria process_spot_products -> fn_process_raw_v2 (DEFEITO 🔴).
--           Dois cron jobs (jobid 1 process-pending-products e jobid 2
--           process-marked-products) no mesmo schedule */5 processavam o
--           MESMO fornecedor concorrentemente. A v2 abria o batch via EXISTS
--           antes do loop; sob concorrência o "perdedor" abria batch e o loop
--           não encontrava linhas -> ~56k batches vazios (99,98%).
-- SOLUÇÃO:
--   1) pg_try_advisory_xact_lock por fornecedor em fn_process_raw_v2.
--   2) abertura LAZY do batch (só cria batch quando há >=1 parent real).
--   3) process_pending_batches simplificado (1 chamada, sem loop redundante).
--   4) remover o cron duplicado (jobid 2 / process-marked-products).
--   5) purge one-time dos batches vazios (ver bloco DO ao final).
-- ============================================================
...
BEGIN
    ...
    -- (dois cron jobs no mesmo tique). O perdedor sai sem criar batch nem trabalho.
    ...
    FOR v_parent IN
        SELECT DISTINCT raw_data->>v_parent_key_source AS pkey
         ...
         LIMIT p_batch_size
    LOOP
      -- Abertura LAZY do batch: só na primeira iteração real (>=1 parent existe)
      IF NOT v_batch_open THEN
...
COMMENT ON FUNCTION public.fn_process_raw_v2(uuid,integer,boolean) IS
    'v2.1 2026-06-04: + pg_try_advisory_xact_lock por fornecedor (anti-race) e abertura LAZY de batch (não cria batch vazio). Demais comportamentos inalterados.';

-- process_pending_batches: 1 chamada à v2 (sem loop redundante por batch)
...
DECLARE
    v jsonb;
BEGIN
    -- CUTOVER (forward-only): Spot processado pelo motor blindado v2.
    -- Uma única chamada processa todas as raw pendentes do fornecedor (até 1000).
    v := fn_process_raw_v2('bcfc0d02-44c6-48ae-8472-12b1a3f3d8e0'::uuid, 1000, true);
    RETURN QUERY SELECT
        NULLIF(v->>'batch_id','')::uuid,
        COALESCE((v->>'variants_processed')::integer, 0),
        CASE WHEN COALESCE((v->>'success')::boolean, false) THEN 'SUCCESS' ELSE 'ERROR' END;
END;
$function$;

-- Remover o cron duplicado (process-marked-products). O orquestrador
-- process-pending-products (jobid 1) já cobre o mesmo fornecedor via v2.
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM cron.job WHERE jobname='process-marked-products') THEN
        PERFORM cron.unschedule('process-marked-products');
    END IF;
END $$;

-- Purge one-time dos batches vazios e não-referenciados do SPOT (defeito histórico).
DELETE FROM supplier_import_batches b
 WHERE b.supplier_id='bcfc0d02-44c6-48ae-8472-12b1a3f3d8e0'
   AND COALESCE(b.products_imported,0)=0
   AND COALESCE(b.variants_imported,0)=0
   AND NOT EXISTS (SELECT 1 FROM supplier_products_raw r WHERE r.import_batch_id=b.id);
