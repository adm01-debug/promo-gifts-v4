...
BEGIN
  ...
  ELSIF r.supplier_id=v_XBZ THEN
    ...
    v_cost:=public.fn_safe_num(r.raw_data->>'PrecoVenda');   -- PrecoVenda do XBZ = CUSTO (confirmado na VSS)
  ...
  ELSIF r.supplier_id=v_ASIA THEN
    ...
    v_cost:=public.fn_safe_num(r.raw_data->>'preco');       -- preco da Asia = CUSTO
  ...
  RETURN jsonb_build_object('success',true,'variante_id',v_var_id,'parent',v_parent,'cor',v_fname,'custo',v_cost);
END;
$$;
