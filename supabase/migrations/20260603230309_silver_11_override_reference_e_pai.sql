...
-- Refactor aditivo: fn_standardize_raw aceita override de referência (para o PAI).
-- Drop da versão de 1 parâmetro e recriação com p_override_reference DEFAULT NULL
-- (chamadas antigas fn_standardize_raw(id) continuam válidas via default).
DROP FUNCTION IF EXISTS public.fn_standardize_raw(uuid);
...
BEGIN
  ...
  FOR m IN
    ...
    ORDER BY priority
  LOOP
    IF m.source_path IS NOT NULL THEN v_val := r.raw_data #>> string_to_array(m.source_path,'.');
    ELSE v_val := r.raw_data ->> m.source_field; END IF;
...
-- Padroniza o PAI a partir de uma raw representante do grupo (mesmos dados de produto).
...
BEGIN
  ...
  RETURN public.fn_standardize_raw(v_rep, p_parent_reference);
END;
$$;
