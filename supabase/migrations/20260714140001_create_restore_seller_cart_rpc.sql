-- ============================================================
-- BUG FIX: cria RPC restore_seller_cart(_snapshot jsonb)
-- Criada pelo Lovable em 20260713101342 mas nunca aplicada ao banco canônico.
-- Causa dos bugs no console:
--   BUG 1: POST /rpc/restore_seller_cart → 404 (função não existia)
--   BUG 2: fallback tentava INSERT com shipping_deadline → PGRST204
-- ============================================================
...
BEGIN
  ...
  INSERT INTO public.seller_carts (
    seller_id, company_id, company_name, company_location, company_logo_url,
    notes, status, shipping_deadline
  )
  VALUES (
    _uid,
  ...
  SELECT COALESCE(jsonb_agg(row_to_json(dedup)::jsonb), '[]'::jsonb), COUNT(*)::int
  ...
  IF jsonb_array_length(_items_deduped) > 0 THEN
    ...
    SELECT
      _new_cart_id,
      row_data->>'product_id', row_data->>'product_name', row_data->>'product_sku',
      row_data->>'product_image_url',
      COALESCE((row_data->>'product_price')::numeric, 0),
      COALESCE((row_data->>'quantity')::int, 1),
      row_data->>'color_name', row_data->>'color_hex', row_data->>'notes',
...
COMMENT ON FUNCTION public.restore_seller_cart(jsonb) IS
  ...
  'Criada em Lovable/20260713101342, aplicada ao banco em 2026-07-14.';
