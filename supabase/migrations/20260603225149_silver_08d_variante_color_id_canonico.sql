...
DECLARE
  ...
  v_fname text; v_fcode text; v_fhex text;   -- valores finais de cor (descritivos)
  v_canonical_id uuid;                        -- color_variations.id (FK de product_variants)
...
BEGIN
  ...
  ELSIF r.supplier_id=v_ASIA THEN
    ...
    v_stock:=public.fn_safe_int(r.raw_data->>'var_estoque'); v_sku:='ASIA-'||r.supplier_reference;
  ELSE
  ...
  -- resolução do fornecedor (descritivos): nome/código/hex
  ...
  -- resolução CANÔNICA (color_variations.id) por nome>hex; NULL se não casar (opção a)
  ...
  RETURN jsonb_build_object('success',true,'variante_id',v_var_id,'parent',v_parent,
                            'cor',v_fname,'color_id_canonico',v_canonical_id,
                            'casou_canonica',(v_canonical_id IS NOT NULL));
END;
$$;
