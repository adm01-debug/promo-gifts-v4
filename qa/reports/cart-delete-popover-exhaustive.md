...
**Modo:** validação — nenhum arquivo de produção foi alterado.
...
|----------------------------------|-----------:|-------:|-------:|
| Unit (`vitest`, handler contract)|         11 |     11 |      0 |
| Fuzz determinístico (seeds 1..500) |       500 |    500 |      0 |
| E2E (Playwright, exhaustive)     |         15 |      — |      — |
| Auditoria RLS/DB (banco canônico)|          3 |      3 |      0 |

- **Unit + fuzz:** executados aqui, 100% verdes.
- **E2E:** especificação criada (`e2e/ui/cart-delete-header-popover-exhaustive.spec.ts`, 15 cenários). Não executada no sandbox porque a suíte exige o dev-server autenticado e credenciais `E2E_*` — deve rodar via `npm run test:e2e` no CI.

**Veredito:** o fix está semanticamente correto. Nenhuma invariante do modelo foi violada em 500 execuções fuzz cobrindo 12.760 ações totais. Contrato dos handlers do JSX é auditado por unit e não regride sem quebrar teste.
...
**Ordem crítica auditada** pelo unit `setOpen(false) é chamado ANTES`. Se um refactor
trocar a ordem, o teste falha. Comportamento em SSR/ambiente sem `requestAnimationFrame`
é coberto pelo unit "quando rAF não roda o callback": nesse caso o dialog não abre —
degradação segura (não regride para o bug de "nada acontece").
...
**Modo de erro do backend:** 25% de falhas (500) aleatórias.
...
**Total de DELETE tentados:** 520. **Sucessos:** 252. Taxa observada compatível com 75% ok / 25% fail. Nenhum seed exigiu repro.
...
Server-side não introduz gap. O bug reportado ("nem exclui, nem aparece a validação") era 100% client-side (corrida de foco entre DismissableLayers).
...
- `.eq('id', cartId)` — não vulnerável a delete em massa. ✓
- Sem `onMutate` optimista — o cartão não some antes do 204. ✓
- Invalidação só após sucesso — coerente com E2E #7 (falha persistente mantém lista). ✓

---

## 6. Especificação E2E (15 cenários)
...
|-----|------|--------------|
| 🔵 Info | O handler da lixeira depende de `requestAnimationFrame`. Em ambientes SSR/JSDOM sem polyfill, o dialog silenciosamente não abriria. | Adicionar fallback `if (typeof requestAnimationFrame === 'function') raf(...) else setPendingDeleteId(id)` — só relevante se o botão vier a ser renderizado sob teste jsdom. |
...
# E2E (requer credenciais E2E_* + dev-server)
npx playwright test e2e/ui/cart-delete-header-popover-exhaustive.spec.ts --project=chromium
```
