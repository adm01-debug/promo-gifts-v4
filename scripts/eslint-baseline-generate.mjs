...
function aggregate(report) {
  ...
  for (const file of report) {
    ...
    for (const m of file.messages) {
      // Congela erros (severity 2) E warnings (severity 1) — o checker
      // (check-eslint-baseline.mjs) compara ambas as severidades por file:rule.
      // Congelar só erros faria todo warning legado virar "regressão".
      if (m.severity === 0) continue;
      const rule = m.ruleId ?? "<no-rule>";
      counts[rel] ??= {};
      counts[rel][rule] = (counts[rel][rule] ?? 0) + 1;
      if (m.severity === 2) totalErrors += 1;
      else if (m.severity === 1) totalWarnings += 1;
    }
  }
