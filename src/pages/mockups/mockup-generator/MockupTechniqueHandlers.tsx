...
} from '@/components/mockup/techniqueColorUtils';
...
export function useTechniqueHandlers({
  ...
  const handleTechniqueChange = useCallback(
    (technique: MockupTechnique | null) => {
      ...
      if (technique && techniqueNeedsColorConfig(technique.name, technique.code)) {
        ...
        setTechniqueColorConfig({
          category: classifyTechnique(technique.name, technique.code),
          isFullColor: true,
        });
      } else {
        setTechniqueColorConfig(null);
  ...
  const confirmTechniqueChange = useCallback(() => {
    ...
    setTechniqueChangeDialogOpen(false);
    toast.info(
      `Técnica alterada para ${pendingTechnique.name}. Dimensões ajustadas automaticamente.`,
      { duration: 3000 },
    );
