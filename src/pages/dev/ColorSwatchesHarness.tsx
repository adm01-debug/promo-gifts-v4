    ...
    <TooltipProvider>
      <main className="min-h-screen bg-background p-4">
        ...
        <p className="mb-4 text-xs text-muted-foreground">
          Redimensione o viewport para 320px e confirme: (a) nenhuma bolinha cortada nas bordas;
          (b) todas as cores visíveis em modo wrap; (c) chip "+N" apenas no modo legado.
        </p>

        {(['wrap', 'legado'] as const).map((mode) => (
          <section key={mode} className="mb-6">
            <h2 className="mb-2 text-sm font-semibold uppercase tracking-wide text-muted-foreground">
              Modo {mode === 'wrap' ? 'wrap (grid Catálogo/Super Filtro/Novidades/Reposição)' : 'legado (lista/tabela)'}
            </h2>
