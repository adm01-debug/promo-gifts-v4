...
export async function handleSyncBitrix(params: {
  ...
  logQuoteHistory: (qId: string, action: string, desc: string, meta?: Record<string, unknown>) => Promise<void>;
  ...
  if (!effectiveBitrixCompanyId) {
    toast.error("Empresa sem ID Bitrix24", { description: "Esta empresa não possui um vínculo com o Bitrix24." });
    return;
  }
