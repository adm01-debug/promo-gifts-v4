...
export interface OAuthErrorCopy {
  /** Codigo normalizado para debug/QA. */
  code?: string;
...
const RAW_MAP: Record<string, OAuthErrorCopy> = {
  access_denied: {
    ...
    description:
      'Você cancelou a autorização na tela do Google ou negou as permissões solicitadas.',
  ...
  redirect_uri_mismatch: {
    ...
    description:
      'A URL para onde o Google deveria voltar não está cadastrada no console do provedor.',
  ...
  signup_disabled: {
    ...
    description:
      'Esta plataforma é restrita — novos usuários precisam ser provisionados manualmente.',
  ...
  state_mismatch: {
    ...
    description:
      'O parâmetro de segurança (state) não bateu — possivelmente o login demorou demais.',
  ...
  provider_not_enabled: {
    ...
    description:
      'Estamos preparando a conexão com o Google para esta conta. Por enquanto, use seu e-mail e senha cadastrados.',
...
export function resolveOAuthError(raw: string | null | undefined): OAuthErrorCopy {
  ...
  if (RAW_MAP[normalized]) return { ...RAW_MAP[normalized], code: normalized };

  // 2) match por substring (descriptions vindas do provider)
  for (const [key, copy] of Object.entries(RAW_MAP)) {
    if (normalized.includes(key)) return { ...copy, code: key };
  }
