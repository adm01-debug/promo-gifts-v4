...
export const ProductWebhookPayloadSchema = z
  .object({
  ...
  .superRefine((val, ctx) => {
    ...
    if (
      (val.action === 'sync' || val.action === 'batch_upsert') &&
...
export const DispatcherBodySchema = z
  .object({
...
export const InboundWebhookEnvelopeSchema = z.object({
  ...
  signature: z
    .string()
    .regex(/^(sha256=)?[a-f0-9]{64}$/i, 'signature must be hex sha256, optionally prefixed')
    .optional(),
});
