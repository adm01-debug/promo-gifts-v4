  ...
  const test = useCallback(
    async (
      ...
      if (
        ...
        'env_key' in optionsOrConfig ||
        'connectionId' in optionsOrConfig ||
        'silent' in optionsOrConfig
      ) {
      ...
      setIsTesting(true);
      const log = createClientLogger('connections.testCredentials', {
        base: { type, env_key, connection_id },
      });
      log.info('test_start');
      try {
        ...
        if (!silent) {
          if (normalized.ok) {
            toast.success('Conexão OK', {
              description:
                sanitizeMessage(normalized) ??
                `${normalized.status ?? '200'} em ${normalized.latency_ms ?? '?'}ms`,
            });
          } else {
