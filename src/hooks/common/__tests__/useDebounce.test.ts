...
describe('useDebounce', () => {
  ...
  it('reinicia timer quando valor muda antes do delay', () => {
    ...
    rerender({ val: 'b' });
    act(() => { vi.advanceTimersByTime(200); }); // 200ms de 300ms
    rerender({ val: 'c' }); // muda antes de completar
    act(() => { vi.advanceTimersByTime(300); }); // completa 300ms desde 'c'
...
describe('useThrottle — BUG-09 leading-edge correto', () => {
  ...
  it('BUG-09: primeira mudanca emite IMEDIATAMENTE (nao debounce)', () => {
    const { result, rerender } = renderHook(
      ({ val }: { val: string }) => useThrottle(val, 500),
      { initialProps: { val: 'a' } }
    );
    // Mudar o valor — deve emitir de imediato (leading-edge), não após 500ms
    act(() => { rerender({ val: 'b' }); });
  ...
  it('durante o lock: trailing update apos o periodo', () => {
    const { result, rerender } = renderHook(
      ({ val }: { val: string }) => useThrottle(val, 300),
      { initialProps: { val: 'a' } }
    );
    act(() => { rerender({ val: 'b' }); }); // leading: emite 'b'
    act(() => { rerender({ val: 'c' }); }); // durante lock: bufferiza 'c'
    act(() => { rerender({ val: 'd' }); }); // durante lock: bufferiza 'd' (overwrite)
    act(() => { vi.advanceTimersByTime(300); }); // trailing: emite 'd'
...
describe('useSearchAsYouType — BUG-24 onSearch via ref', () => {
  ...
  it('BUG-24: mudar onSearch entre renders nao re-dispara effect', () => {
    ...
    rerender({ cb: () => { callCount += 100; } }); // nova ref do callback
