...
//  [v2.2] 9. BUG-FIX CRÍTICO: supabase.rpc.bind(supabase) — restaura `this` binding
//             v2.1 extraiu rpc como variável sem .bind(), causando perda de contexto:
//             SupabaseClient.prototype.rpc() internamente faz `this.rest.rpc(...)`
//             Com `this === undefined` (ES modules strict mode) → throws sincrono:
//             "TypeError: Cannot read properties of undefined (reading 'rest')"
//             Efeito cascata: throw síncrono → doFetch() falha < 1ms → dedup limpa
//             → INITIAL_SESSION duplo (imediato localStorage + async network verify)
//             não era deduplicado → 2 erros. Retry (+500ms) gerava o 3º erro.
//             Fix: .bind(supabase) preserva this sem alterar comportamento do cast TS.
  ...
  const fetchUserData = useCallback(async (userId: string) => {
    ...
    const doFetch = async () => {
      ...
      try {
        ...
        //
        // BUG-FIX v2.2 (CRÍTICO): .bind(supabase) OBRIGATÓRIO.
        // Extrair supabase.rpc como variável sem .bind() desvincula o método
        // da instância. SupabaseClient.prototype.rpc() faz `return this.rest.rpc(...)`
