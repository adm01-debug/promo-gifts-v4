...
export function useSecretsManager() {
  ...
  const list = useCallback(async (names?: string[]) => {
    ...
    try {
      ...
      if (error) {
        ...
        toast.error('Erro ao listar credenciais', { description: sanitizeMessage(normalized) });
        return [];
      }
      setListError(null);
  ...
  const getRotationHistory = useCallback(async (name?: string): Promise<RotationHistoryEntry[]> => {
    ...
    if (error) {
      toast.error('Falha ao carregar histórico', { description: sanitizeMessage(error) });
      return [];
    }
