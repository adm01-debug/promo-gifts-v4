...
export function FavoritesHeatmap() {
  ...
  const { data } = useQuery({
    ...
    queryFn: async (): Promise<WeekRow[]> => {
      if (!user) return [];
      const { data: queryRows, error } = await supabase.rpc('get_favorites_weekly_count', {
        _weeks: 8,
      });
  ...
  return (
    <div className="flex items-center gap-3 rounded-lg border border-border bg-card/50 px-3 py-2">
      <div className="flex h-8 items-end gap-0.5" aria-label="Histórico semanal de favoritos">
        {data.map((d, i) => {
