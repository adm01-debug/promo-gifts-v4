...
vi.mock('@/hooks/products/useProductLeafCategories', () => ({ useLeafCategory: () => ({ id: '1', name: 'Cat' }) }));
vi.mock('@/contexts/SellerCartContext', () => ({ useSellerCartContext: () => ({ isInAnyCart: () => false, addToCart: vi.fn() }) }));
vi.mock('@/contexts/CollectionsContext', () => ({ useCollectionsContext: () => ({ collections: [], addToCollection: vi.fn() }) }));
...
describe('ProductCard Layout and Typography', () => {
  it('should have supplier on the left and SKU on the right with correct font size', () => {
    render(<Wrapper><ProductCard product={mockProduct} /></Wrapper>);
