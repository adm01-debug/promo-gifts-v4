            ...
            {isVideo(selectedIndex) ? (
              ...
              <img
                ...
                onError={(e) => {
                  ...
                  if (stage === '0') {
                    // large variant failed → try /public variant
                    img.dataset.fallbackStage = '1';
                    img.src = allMedia[selectedIndex]; // raw CF URL (/public)
                  } else if (stage === '1') {
                    // /public also failed → try supplier origin (SPOT only)
                    const origin = deriveOriginalUrl(allMedia[selectedIndex]);
                    if (origin) { img.dataset.fallbackStage = '2'; img.src = origin; }
                    else { img.src = '/placeholder.svg'; }
                  } else {
                    // origin also failed → placeholder
