...
export function ExportComparisonButton({ products, targetSelector = "#compare-export-area", formatCurrency }: Props) {
  ...
  const exportPNG = async () => {
    ...
    try {
      ...
      if (!el) { toast.error("Área não encontrada"); return; }
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end">
        <DropdownMenuItem onClick={exportPDF}>
          <FileText className="h-4 w-4 mr-2" /> PDF (paisagem)
        </DropdownMenuItem>
        <DropdownMenuItem onClick={exportPNG}>
          <ImageIcon className="h-4 w-4 mr-2" /> PNG (imagem)
        </DropdownMenuItem>
        <DropdownMenuItem onClick={exportCSV}>
          <FileSpreadsheet className="h-4 w-4 mr-2" /> CSV (planilha)
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  );
