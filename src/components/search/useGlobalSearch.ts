    ...
    try {
      ...
      if (intent.type === 'product' || intent.type === 'mixed') {
        try {
          ...
          const fuse = new Fuse(
            filteredProducts as any, // eslint-disable-line @typescript-eslint/no-explicit-any
            createProductFuseOptions<ExternalProduct>(),
          );
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          rankProductSearchResults(filteredProducts as any, productQuery, fuse as any).forEach(
            (p) => {
              allResults.push({
                id: p.id,
                title: p.name,
      ...
      void supabase.auth.getUser().then(
        ({ data }) => {
          ...
          return supabase
            .from('search_analytics' as any) // eslint-disable-line @typescript-eslint/no-explicit-any
            .insert({
              seller_id: sellerId,
