...
interface HeaderProps {
  ...
  /**
   * Drives the menu button's aria-label / aria-expanded so assistive tech
   * announces the sidebar state correctly. Optional — defaults to false so
   * existing callers that don't pass it still see a sensible "Abrir menu"
   * label.
   */
  sidebarOpen?: boolean;
}
...
export const Header = React.memo(function Header({
  onMenuToggle,
  // `searchQuery` / `onSearchChange` arrive from MainLayout but the new
  // Header design uses the global search palette — props are accepted to
  // keep the call sites stable while the legacy search field is retired.
