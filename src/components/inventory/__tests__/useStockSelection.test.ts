...
describe('rowKey', () => {
  ...
  it('chaves diferentes para IDs diferentes', () => {
    expect(rowKey({ productId: 'p1', variantId: 'v1' })).not.toBe(
      rowKey({ productId: 'p1', variantId: 'v2' }),
    );
    expect(rowKey({ productId: 'p1', variantId: 'v1' })).not.toBe(
      rowKey({ productId: 'p2', variantId: 'v1' }),
    );
  });
  it('é determinístico', () => {
...
describe('toggle', () => {
  ...
  it('múltiplos toggles independentes', () => {
    ...
    act(() => {
      result.current.toggle(k1);
      result.current.toggle(k2);
    });
  ...
  it('toggle de chave fantasma não quebra selectedRows', () => {
    const { result } = renderHook(() => useStockSelection([]));
    act(() => {
      result.current.toggle('fantasma');
    });
    expect(result.current.selectedRows).toHaveLength(0);
    expect(result.current.selectedCount).toBe(1);
  });
...
describe('selectAllVisible', () => {
  it('seleciona todas as linhas visíveis', () => {
    ...
    const { result } = renderHook(() => useStockSelection(rows));
    act(() => {
      result.current.selectAllVisible(rows);
    });
  ...
  it('substitui seleção prévia', () => {
    ...
    const { result } = renderHook(() => useStockSelection(rows));
    act(() => {
      result.current.toggle(rowKey({ productId: 'p1', variantId: 'v1' }));
    });
    act(() => {
      result.current.selectAllVisible([rows[1]]);
    });
    expect(result.current.selectedCount).toBe(1);
  ...
  it('lista vazia limpa seleção', () => {
    ...
    const { result } = renderHook(() => useStockSelection(rows));
    act(() => {
      result.current.toggle(rowKey({ productId: 'prod-1', variantId: 'var-1' }));
    });
    act(() => {
...
describe('selectedRows derivação', () => {
  it('contém as rows reais das keys', () => {
    ...
    act(() => {
      result.current.toggle(rowKey({ productId: 'p2', variantId: 'v2' }));
    });
...
describe('bulkQuote', () => {
  ...
  it('usa minStock como quantity, fallback 1 se zero', () => {
    ...
    act(() => {
      result.current.selectAllVisible([rowA, rowB]);
    });
    act(() => {
      result.current.bulkQuote();
    });
