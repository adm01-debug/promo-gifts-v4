    ...
    <div className="space-y-3">
      ...
      {(fc7 || fc15 || fc30) && (
        <div className="flex flex-wrap items-center gap-2 rounded-lg border border-border/30 bg-muted/20 px-4 py-2">
          ...
          <span className="text-xs font-medium text-muted-foreground">Previsão de novas rupturas:</span>
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <span className={cn(
                    ...
                    fc!.net_ruptura > 1000 ? 'bg-red-500/15 text-red-600 dark:text-red-400'
                      : fc!.net_ruptura > 300 ? 'bg-orange-500/15 text-orange-600'
                ...
                <TooltipContent className="text-xs">
                  ...
                  <div className="text-emerald-400">{fc!.com_restock.toLocaleString('pt-BR')} têm reposição prevista</div>
                  <div className="font-bold text-red-400">{fc!.net_ruptura.toLocaleString('pt-BR')} risco líquido</div>
              ...
              <TooltipProvider delayDuration={100}><Tooltip>
                <TooltipTrigger asChild>
                  <span className="flex cursor-help items-center gap-0.5 rounded-full bg-amber-500/10 px-2 py-0.5 text-xs font-semibold text-amber-600">
                    <Flame className="h-3 w-3" /> {spikesAtivos.valor.toLocaleString('pt-BR')} picos
                  </span>
                </TooltipTrigger>
              ...
              <TooltipProvider delayDuration={100}><Tooltip>
                <TooltipTrigger asChild>
                  <span className="flex cursor-help items-center gap-1">
                    ...
                    EMA {pctEma.valor}%
                    {pctAltaConf && <span className="text-muted-foreground/60">· conf {pctAltaConf.valor}%</span>}
                  </span>
                </TooltipTrigger>
                <TooltipContent>
              ...
              return (
                <button
                  ...
                  className={cn('flex flex-col items-start gap-1 rounded-lg border p-3 text-left transition-all',
                    cfg.border, isActive ? cfg.active : cfg.bg, count === 0 && 'cursor-not-allowed opacity-40')}
