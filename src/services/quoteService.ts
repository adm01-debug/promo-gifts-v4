...
export const quoteService = {
  async fetchQuotes(userId: string, scope: string) {
    let query = supabase
      // rls-allow: seller scope aplicado condicionalmente abaixo (scope 'self' → .eq seller_id); admin scope 'all' protegido por RLS
      .from('quotes')
      .select('*')
  ...
  async fetchQuote(quoteId: string): Promise<Quote | null> {
    const { data: quoteData, error: qErr } = await supabase
      // rls-allow: fetch por id único; ownership do registro garantido por RLS
      .from('quotes')
      .select('*')
      .eq('id', quoteId)
      .single();
  ...
  async createQuote(
    ...
    const { data: inserted, error: insErr } = await supabase
      // rls-allow: insert com seller_id definido em buildInsertPayload; RLS valida o seller_id inserido
      .from('quotes')
  ...
  async updateQuote(quoteId: string, quote: Partial<Quote>, items: QuoteItem[]): Promise<Quote> {
    ...
    const { data: updated, error: updErr } = await supabase
      // rls-allow: update por id único; ownership do registro garantido por RLS
      .from('quotes')
  ...
  async deleteQuote(quoteId: string) {
    // rls-allow: delete por id único; ownership do registro garantido por RLS
