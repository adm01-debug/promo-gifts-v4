...
export function mapPromobrindToProduct(p: PromobrindProduct): Product {
  ...
  return {
    ...
    // URL fallback para CF outage — banco Gold > enrichProducts runtime > null
    // primary_image_fallback_url é coluna Gold denormalizada mantida por trigger
    // disponível em 100% dos fetches (lightweight, full, search, >500 produtos)
    // cobrindo todos os 5 fornecedores sem depender do enriquecimento de imagens
    primaryImageFallbackUrl: p.primary_image_fallback_url ?? p.primaryImageFallbackUrl ?? null,
